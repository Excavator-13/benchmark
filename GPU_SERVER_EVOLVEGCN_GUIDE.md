...
- 论文文字说数据按 9:1:2 划分，释放代码实际在窗口化后按 0.83/0.04/剩余部分切分。当前实现保留名义比例、测试期与验证样本数，但在两个边界各跳过 `pred_length - 1` 个窗口，并从训练样本数中扣除这些间隔，避免目标月份泄漏。默认 36 个月、窗口 6、预测 3 时实际为 19/1/4 个快照：训练目标截至 2023-03，验证为 2023-04 至 06，测试为 2023-07 至 12。旧切分的指标不能直接与新结果比较。
